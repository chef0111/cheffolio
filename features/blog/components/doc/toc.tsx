...
const ObserverContext = React.createContext<Observer | null>(null);
...
function useObserver() {
  const observer = React.use(ObserverContext);
...
export function useTOCListener(listener: ChangeListener) {
  ...
  const callback = React.useEffectEvent(listener);
...
export function useTOCSelector<T>(
  ...
  const [value, setValue] = React.useState<T>(() => select(observer.items));
