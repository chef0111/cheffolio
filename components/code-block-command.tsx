...
export function CodeBlockCommand({
  ...
  const tabsFiltered = useMemo(
    () => Object.entries(tabs).filter(([, value]) => Boolean(value)),
    [tabs]
  );

  const available = tabsFiltered.map(([key]) => key);
  const active = available.includes(packageManager)
    ? packageManager
    ...
    <div className="bg-code relative overflow-hidden rounded-xl">
      ...
      >
        <ScrollArea.Root className="shadow-border w-full pr-10 shadow-[inset_0_-1px_0_0]">
          ...
          >
            <IconSwap>
              <IconSwapItem className="mr-2" key={active}>
                {getIconForPackageManager(active)}
              </IconSwapItem>
            </IconSwap>
