...
import type { HonoStack } from "#/stack/types";
...
function serverEntry(stack: HonoStack): string {
  ...
  if (stack.database !== "none" && stack.api === "orpc") {
    ...
    lines.push(`  const { matched, response } = await handler.handle(c.req.raw, {`);
    lines.push(`    prefix: "/api",`);
    lines.push(`    context: { headers: c.req.raw.headers, userId: c.get("userId") },`);
    lines.push(`  });`);
...
function webDependencies(stack: HonoStack, dep: string): Record<string, string> {
  const dependencies: Record<string, string> = {
    "@repo/ui": dep,
    react: "^19.1.1",
    "react-dom": "^19.1.1",
